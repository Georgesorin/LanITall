...
# HIT ZONES EXPANDED FOR BETTER VERTICAL FORGIVENESS
HIT_ZONE_BOTTOM = (22, 31) # Zone mult mai mari pe axa Y
HIT_ZONE_TOP    = (0, 9)
FLASH_DURATION  = 0.20
...
class PianoTilesEngine:
    ...
    def process_inputs(self):
        ...
        for i in range(512):
            if self.button_states[i] and not self.prev_button_states[i]:
                ...
                # FAT FOOT HITBOX: Horizontal Forgiveness
                # Verifica daca a calcat pe coloana, sau la +/- 1 tile distanta (stanga/dreapta)
                target_col_idx = None
                
                if x in active_cols:
                    target_col_idx = active_cols.index(x)
                else:
                    # Cauta daca e langa o coloana activa
    ...
    def _try_hit(self, col, current_t):
        ...
        for tile_id, tile in enumerate(self.beatmap):
            ...
            # Expanded general time allowance (was 0.3)
    ...
    def _update_logic(self):
        ...
        for tile_id, tile in enumerate(self.beatmap):
            # Asteptam 0.4 secunde sa ne asiguram ca a iesit sigur din zona de hit
            if song_time > tile['time'] + 0.4:
                col = tile['column']
                if self.mode == '1':
